        ...
        return new Promise(function(resolve, reject) {
            function next() {
                if (index < migrationCommands.length)
                {
                    ...
                    console.log("[#"+index+"] execute: " + command.fn); // eslint-disable-line no-console
                    index++;
...
const fakeMigration = function(queryInterface, filename, file, cb) {
    let mig = require(filename);

    if (!mig)
        return cb('Can\'t require file ' + filename);

    queryInterface.bulkInsert('SequelizeMeta', [
        {name: file},
    ]).then(function() {
        cb()
    }).catch(function(e) {
...
module.exports = { writeMigration, getMigration, sortActions, parseDifference, reverseModels, executeMigration, fakeMigration };
